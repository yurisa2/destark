#!/usr/bin/env python3
"""
Command-line interface for the Parallel Raster Fuzzy Inference System.
...
from raster_fuzzy_system_parallel import ParallelRasterFuzzyInferenceSystem, create_raster_config_template
import json
...
def main():
    parser = argparse.ArgumentParser(
        description="Parallel Raster Fuzzy Inference System - Process 3 TIFF files into 1 output TIFF using multiple CPU cores",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create a configuration template
  python run_raster_fis_parallel.py --create-config

  # Process TIFF files with default parallel settings (all CPU cores)
  python run_raster_fis_parallel.py social.tif environmental.tif strategic.tif output.tif

  # Process TIFF files with custom number of CPU cores
  python run_raster_fis_parallel.py social.tif environmental.tif strategic.tif output.tif --cores 8

  # Process TIFF files with custom chunk size and cores
  python run_raster_fis_parallel.py social.tif environmental.tif strategic.tif output.tif --cores 4 --chunk-size 200

  # Process TIFF files with custom configuration and NoData value
  python run_raster_fis_parallel.py social.tif environmental.tif strategic.tif output.tif --config my_config.json --nodata -32768
        """
    )
    ...
    parser.add_argument('--cores', '-p', type=int, default=None,
                       help=f'Number of CPU cores to use (default: all available, max: {mp.cpu_count()})')
    ...
    try:
        # Initialize the parallel fuzzy inference system
        print(f"Loading configuration from: {args.config}")
        fis = ParallelRasterFuzzyInferenceSystem(args.config)
        
        # Process the rasters in parallel
        print(f"Processing rasters with parallel processing...")
