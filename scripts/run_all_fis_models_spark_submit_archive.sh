...
# Script that uses spark.yarn.archive to bypass HDFS completely
set -e
...
# Find code directory and Spark home
CODE_DIR="/mnt/destark"
SPARK_HOME="/usr/lib/spark"
cd "$CODE_DIR"

# Create Spark JARs archive to avoid HDFS upload
echo "=== Creating Spark JARs archive ==="
if [ ! -f "/tmp/spark-jars.zip" ]; then
    echo "Creating spark-jars.zip to avoid HDFS upload..."
    cd /usr/lib/spark/jars
    zip -r /tmp/spark-jars.zip . > /dev/null 2>&1
    cd "$CODE_DIR"
    echo "✓ Created spark-jars.zip"
else
    echo "✓ spark-jars.zip already exists"
fi
...
echo "=== Running All FIS Models with Spark Submit (Archive, YARN cluster) ==="
...
declare -A FIS_CONFIGS=(
    ["max"]="s3://adveng-pipeline-unifile/unifile_test/config_max.json"
    ["median"]="s3://adveng-pipeline-unifile/unifile_test/config_median.json"
    ["minimum"]="s3://adveng-pipeline-unifile/unifile_test/config_minimum.json"
    ["mode"]="s3://adveng-pipeline-unifile/unifile_test/config_mode.json"
    ["round_up"]="s3://adveng-pipeline-unifile/unifile_test/config_round_up.json"
    ["round_down"]="s3://adveng-pipeline-unifile/unifile_test/config_round_down.json"
)
...
run_model() {
    ...
    local config_path="$2"
    local output_file="$3"
    echo "=== Submitting $model_name model (archive, nohup background) ==="
    echo "Config: $config_path"
    echo "Output: $output_file"
    nohup spark-submit \
        --master yarn \
        --deploy-mode cluster \
        --conf spark.yarn.archive="file:///tmp/spark-jars.zip" \
        ...
        --verbose \
        --block-size 50 \
        > logs/${model_name}_archive.log 2>&1 &
    echo "✓ $model_name submitted (archive, PID: $!)"
    echo ""
}
...
echo "=== All Models Submitted in Background (archive) ==="
echo "Jobs will continue even if you disconnect the terminal."
echo "Monitor logs in the logs/ directory or with: tail -f logs/<model>_archive.log"
