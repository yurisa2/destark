...
# Final version that completely bypasses HDFS
set -e
...
echo "=== Running All FIS Models with Spark Submit (Final, YARN cluster) ==="
...
run_model() {
    ...
    local config_path="$2"
    local output_file="$3"
    echo "=== Submitting $model_name model (final, nohup background) ==="
    ...
    nohup spark-submit \
        ...
        --verbose \
        --block-size 50 \
        > logs/${model_name}_final.log 2>&1 &
    echo "✓ $model_name submitted (final, PID: $!)"
    echo ""
}
...
echo "=== All Models Submitted in Background (final) ==="
echo "Jobs will continue even if you disconnect the terminal."
echo "Monitor logs in the logs/ directory or with: tail -f logs/<model>_final.log"
