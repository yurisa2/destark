...
# Files-based version that bypasses HDFS completely
set -e
...
echo "=== Running All FIS Models with Spark Submit (Files-based, YARN cluster) ==="
...
run_model() {
    ...
    local config_path="$2"
    local output_file="$3"
    echo "=== Submitting $model_name model (files-based, nohup background) ==="
    echo "Config: $config_path"
    echo "Output: $output_file"
    nohup spark-submit \
        --master yarn \
        --deploy-mode cluster \
        --files "$CODE_DIR/app/raster_fuzzy_spark_ultra_optimized.py" \
        ...
        raster_fuzzy_spark_ultra_optimized.py \
        "$SOCIAL_TIFF" \
        "$ENVIRONMENTAL_TIFF" \
        "$STRATEGIC_TIFF" \
        "$output_file" \
        ...
        --verbose \
        --block-size 50 \
        > logs/${model_name}_files.log 2>&1 &
    echo "✓ $model_name submitted (files-based, PID: $!)"
    echo ""
}
...
echo "=== All Models Submitted in Background (files-based) ==="
echo "Jobs will continue even if you disconnect the terminal."
echo "Monitor logs in the logs/ directory or with: tail -f logs/<model>_files.log"
