#!/usr/bin/env python3
"""
Spark FIS Processing - EMR Simple Version
Uses EMR's built-in Spark and packages
"""
...
# Try to import packages that should be available on EMR
try:
    import rasterio
except ImportError:
    print("Installing rasterio...")
    os.system("pip3 install rasterio")
    import rasterio
...
try:
    ...
    os.system("pip3 install scikit-fuzzy")
    import skfuzzy as fuzz
    from skfuzzy import control as ctrl
...
try:
    ...
    print("ERROR: PySpark not available. This script must run on EMR with Spark installed.")
    sys.exit(1)
...
def process_config_spark(config_name):
    ...
    # Create Spark session using EMR's built-in Spark
    ...
    num_partitions = max(50, len(chunks) // 10)
...
def main():
    ...
    print("This will process all configurations using distributed Spark")
