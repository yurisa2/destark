...
# Run All FIS Models using Pure Python (No Spark)
# This script runs all available FIS configurations using pure Python processing
# to avoid Java/Spark compatibility issues
...
echo "=== Running All FIS Models with Pure Python ==="
...
# Function to run pure Python processing for a single model
run_pure_python_model() {
    local model_name="$1"
    local config_path="$2"
    local output_file="$3"
    
    echo "=== Processing $model_name model with Pure Python ==="
    ...
    # Run the processing with pure Python (no Spark)
    echo "Starting pure Python processing..."
    start_time=$(date +%s)
...
for model_name in "${!FIS_CONFIGS[@]}"; do
    ...
    # Run the model
    if ! run_pure_python_model "$model_name" "$config_path" "$output_file"; then
