...
# Robust version for EMR cluster with node failure handling
set -e
...
echo "=== Running All FIS Models with Spark Submit (Robust, nohup background) ==="
...
run_model() {
    ...
    local config_path="$2"
    local output_file="$3"
    echo "=== Submitting $model_name model (robust, nohup background) ==="
    ...
    echo "✓ $model_name submitted (robust, PID: $!)"
    echo ""
}
...
echo "=== All Models Submitted in Background (robust) ==="
