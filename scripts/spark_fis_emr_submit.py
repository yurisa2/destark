...
def process_config_spark_submit(config_name):
    """Process a single config with proper Spark submit distribution."""
    print(f"\n=== Processing {config_name} with Spark Submit FIS ===")
    ...
    print(f"Processing {height}x{width} raster with Spark Submit FIS...")
    
    # Create Spark session with proper cluster configuration
    spark = SparkSession.builder \
        .appName(f"FIS-Submit-{config_name}") \
    ...
    # Process with Spark - force more partitions for better distribution
    num_partitions = max(50, len(chunks) // 10)  # More partitions for better distribution
    ...
    output_name = f"result_spark_submit_{config_base}.tif"
...
def main():
    print("=== SPARK SUBMIT FIS PROCESSING STARTED ===")
    ...
    for i, config in enumerate(configs, 1):
        ...
        try:
            process_config_spark_submit(config)
            print(f"SUCCESS: {config}")
        except Exception as e:
            print(f"ERROR: {config} - {e}")
            continue
    
    print("\n=== SPARK SUBMIT FIS PROCESSING COMPLETED ===")
