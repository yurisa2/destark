...
# No-HDFS version that uses local jars instead of HDFS uploads
set -e
...
echo "=== Running All FIS Models with Spark Submit (No-HDFS, YARN cluster) ==="
...
run_model() {
    ...
    local config_path="$2"
    local output_file="$3"
    echo "=== Submitting $model_name model (no-HDFS, nohup background) ==="
    ...
    nohup spark-submit \
        ...
        --verbose \
        --block-size 50 \
        > logs/${model_name}_nohdfs.log 2>&1 &
    echo "✓ $model_name submitted (no-HDFS, PID: $!)"
    echo ""
}
...
echo "=== All Models Submitted in Background (no-HDFS) ==="
echo "Jobs will continue even if you disconnect the terminal."
echo "Monitor logs in the logs/ directory or with: tail -f logs/<model>_nohdfs.log"
